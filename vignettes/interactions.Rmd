...
This avoids redundant permutations such that an interaction between regions #1 and #2 is treated the same as an interaction between regions #2 and #1.
We recommend running `swapAnchors` prior to comparing between interactions within or between `GInteractions` objects, unless you have a good reason not to do so.
...
We also coerce the permutations to a consistent format with `swapAnchors` for comparing between objects.
