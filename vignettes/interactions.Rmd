...
For example, say we have a `all.regions` object containing consecutive intervals (though any regions can be used here, consecutive intervals are just simpler to explain):
...
Note that the exact order doesn't matter -- here, an interaction between regions A and B is considered to be the same as that between B and A.
Anyway, our set of three interactions can be represented with:
...
The first is that the `GRanges` returned by `regions` is sorted -- this is automatically performed within the `GInteractions` class, and is enforced throughout for consistency.
...
This is intentional and avoids redundancy from different permutations of the anchor regions.
...
However, both methods require that the common regions be the same across all objects.
...
The `match` method looks for entries in the second object with the same anchor indices as each entry in the first object.
...
Finally, `GInteractions` objects can also be compared in a parallel manner.
...
The `overlapsAny`, `countOverlaps` and `subsetByOverlaps` methods are also available.
...
We want to determine if any of our "new" `GInteractions` interactions overlap with the existing interaction, e.g., to identify corresponding interactions between data sets.
...
Of all the interactions in our `GInteractions` object, only `gi[2]` is considered to be overlapping, 
    despite the fact that several interactions have 1D overlaps with `of.interest`.
This is because only `gi[2]` has a concomitant overlap with the interacting partner region on chromosome B.

The above call uses a `GRangesList` of length 2, where each constituent `GRanges` has a single interval.
Multiple pairwise interactions can be specified with longer constituent `GRanges`, 
    where corresponding entries between the two `GRanges` represent one existing interaction.
...
Bounding box identification can be done using the `boundingBox` function, as shown below:

```{r}
all.chrs <- as.character(seqnames(regions(gi)))
group.by.chr <- paste(all.chrs[anchors(gi, type="first", id=TRUE)],
                      all.chrs[anchors(gi, type="second", id=TRUE)])
boundingBox(gi, group.by.chr)
```

This function returns a `GRangesList` object where the constituent `GRanges` contain the coordinates for the sides of each bounding box.
The example above identifies the bounding box for all interactions on each pair of chromosomes. 
Note that the bounding box is only defined when all interactions in a group lie on the same pair of chromosomes.
...
The latter is simply the analogue of the former, after replacing genomic intervals in `GRanges` with  pairwise interactions in `GInteractions`.
...
Again, `combine` is distinct from `rbind` or `c` in that objects with different common regions can be combined in the former but not the latter.

Objects with the same interactions can also be combined column-wise with the `cbind` method.
This is used to combine data from different samples.
...
The number and order of rows need not be consistent with that of the columns in each `ContactMatrix`.
...
Any representation of intraction-specific metadata in matrix form would be equivalent to constructing a new `ContactMatrix` 
...
Note that the common regions must be the same in all objects to run `cbind` or `rbind`.
If the common regions differ, they should be set to the same value (namely, a union of common regions from all objects) by using `replaceRegions` beforehand.
The `combine` method is not provided here, as the generic does not provide arguments to specify whether a user would want to combine by row or column.
...
Rows or columns with the same anchor indices will *not* be distinguished by matrix values.
...
Two-dimensional overlaps can also be performed with a `ContrastMatrix` by running `overlapsAny` with a `GRangesList` or `GInteractions` as the `subject`.
This returns a logical matrix indicating which cells of the matrix have overlaps.
...
```{r}
olap <- overlapsAny(cm, GRangesList(of.interest, GRanges("chrB", IRanges(1, 20))))
olap
```
...
This will report pairwise interactions corresponding to each cell of the matrix, with the value of that cell stored as experimental data in the `InteractionSet`:
...
Such duplications may occur when the `ContactMatrix` crosses the diagonal of the interaction space.
...
For example, if sequencing counts were being stored in `iset`, then the linearized data in `rse` could be analyzed as if it represented genomic coverage 
    (where the depth of coverage represents the intensity of the interaction of each region in `rowRanges(rse)` with the region of interest in `x`).
