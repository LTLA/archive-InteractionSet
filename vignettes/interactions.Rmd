...
The `pairdist` function can be applied to a `ContactMatrix`.
This returns a matrix of distances of the same dimension as the `ContactMatrix`.
Each cell contains the distance between the midpoints of the corresponding row/column regions.

```{r}
pairdist(cm)
```

Recall that distances are undefined for inter-chromosomal interactions, so the values are simply set to `NA`.
